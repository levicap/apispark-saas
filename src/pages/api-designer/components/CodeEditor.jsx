...
app.Run();`;
  ...
  }, [isOpen, workflowNodes, language, currentProject]);

  const generatePackageJson = () => {
    if (language === 'javascript') {
      return JSON.stringify({
        name: currentProject?.name?.toLowerCase()?.replace(/\s/g, '-') || 'api-server',
        version: '1.0.0',
        description: currentProject?.description || 'Generated API server',
        main: 'index.js',
        scripts: {
  ...
  const generateReadme = () => {
    return `# ${currentProject?.name || 'API Server'}

${currentProject?.description || 'Generated API server with multiple endpoints'}

## Generated Endpoints

${workflowNodes?.filter(node => ['get', 'post', 'put', 'patch', 'delete']?.includes(node?.type))?.map(node => 
  `- **${node?.type?.toUpperCase()}** \`${node?.data?.endpoint}\` - ${node?.data?.description || 'No description'}`
...
Generated on ${new Date()?.toLocaleDateString()} by API Forge
`;
  };
